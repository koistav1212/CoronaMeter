...
public class recoveredGraph extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view= inflater.inflate(R.layout.fragment_recovered_graph, container, false);
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            int k=0;
                            for(int j=jsonArray1.length()-91;j<=jsonArray1.length()-2;j++) {
                                JSONObject obj1 = jsonArray1.getJSONObject(j);
                                if(obj1.getString("status").equalsIgnoreCase("Recovered"))
                                { yAxisData[k] = Integer.valueOf(obj1.getString("tt"));
                                    axisData[k]=obj1.getString("date").substring(0,6);
                                    k++;
                                }
                            }
                            Line line = new Line(yAxisValues).setColor(Color.parseColor("#0f8719"));
